...
export default function App() {
  ...
  // ─── !! 테스트용 데이터  !!
  // const userFlower = "튤립"; // 예시: 유저가 선택한 꽃의 유형
  // const userColor = "purple"; // 예시: 유저가 선택한 꽃의 색상
  // const userType = "wanderer";
  // const userKeyword = "Budding";
  ...
  useEffect(() => {
    // 1️⃣ URL에서 ?file=xxxxx.json 파라미터 읽기
    const urlParams = new URLSearchParams(window.location.search);
    const resultFile = urlParams.get('file'); // 예: "result_20250607_2136.json"

    if (!resultFile) {
      console.error("❌ result 파일명이 URL에 없습니다.");
      setLoading(false);
      return;
    }

    // 2️⃣ 해당 result_xxx.json 파일 fetch
    fetch(`/results/${resultFile}`)
      .then((res) => {
      ...
      .then((data) => {
        // 3️⃣ 시각화 데이터 저장
        setVisualData({
          scores: data.visual1.scores,
          flower: data.visual1.flower,
          type: data.visual1.type,
          color: data.visual1.color,
          keyword: data.visual1.keyword,
      ...
      .catch((err) => {
        console.error("시각화 데이터 로드 실패", err);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const cleanedVisual3 = visualData.visual3.filter(
    p => p.x !== null && p.y !== null
  );
  ...
  return (
    ...
    <div className="app-container">
      ...
      >
        <Sec5
          bgImage={bgCluster1}
          heatmap={visualData.visual2}
          circleColor="rgba(189, 189, 189, 0.3)" // 원 색⋅투명도 변경 가능
          texts={[
            "Color Selection",
            colorTextMap[visualData.color] ||
              "당신의 시선이 향한 곳은..",
          ]}
        />
      </div>
      ...
      >
        <Sec5
          bgImage={bgCluster2}
          heatmap={cleanedVisual3}
          circleColor="rgba(189, 189, 189, 0.2)" // 원 색⋅투명도 변경 가능
          texts={[
            "INNER KEYWORD",
            keywordTextMap[
              visualData.keyword
